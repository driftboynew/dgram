...
# ── Radio proxy — re-encodes any stream to 16kbps mono for 2G ─────────────────
...
def radio_proxy():
    ...
    try:
        ...
        return Response(generate(), mimetype='audio/mpeg',
                        headers={'Content-Disposition': 'inline; filename="radio.mp3"'})
    except Exception as e:
        return f'Proxy error: {html.escape(str(e))}', 500


# ── YouTube audio client ───────────────────────────────────────────────────────
            ...
            try:
                ...
                if lines:
                    for line in lines:
                        ...
                        title   = html.escape(parts[1].strip()[:50])
                        dur     = parts[2].strip() if len(parts) > 2 else ''
                        channel = html.escape(parts[3].strip()[:30]) if len(parts) > 3 else ''
                        results += f"""<a href="/youtube/play/{vid_id}">
  <div class="row">
    <div class="name">&#9654; {title}</div>
    <div class="preview">{channel} &middot; {dur}</div>
  </div>
</a>"""
                else:
...
<div class="info" style="font-size:10px">
  Audio only. Tap result to download 16kbps MP3.<br/>
  Opens in Nokia music player. Works on 2G.
</div>
<form method="post" action="/youtube">
  <label>Search YouTube:</label>
  <input type="text" name="q" value="{html.escape(query)}" placeholder="Song, artist, topic..."/>
  <input type="submit" value="Search &#9658;"/>
</form>
{results}"""
    return page('YouTube Audio', body, back='/home')
...
def youtube_play(video_id):
    ...
    body = f"""
<div class="info">
  Tap download below.<br/>
  Wait 10-20 seconds for server to prepare.<br/>
  Then play in Nokia music player.
</div>
<a href="/youtube/download/{safe_id}">&#9660; Download 16kbps MP3</a>
<a href="/youtube">&#8592; Back to search</a>"""
    return page('Download', body, back='/youtube')
...
def youtube_download(video_id):
    ...
    def generate():
        try:
            url_proc = subprocess.run(
                ['yt-dlp', f'https://www.youtube.com/watch?v={safe_id}',
                 '-f', 'bestaudio/best', '--get-url',
                 '--no-playlist', '--socket-timeout', '15'],
                capture_output=True, text=True, timeout=30
            )
            audio_url = url_proc.stdout.strip().split('\n')[0]
            if not audio_url:
                yield b'Error: could not get audio URL'
                return
    ...
    return Response(generate(), mimetype='audio/mpeg',
                    headers={'Content-Disposition': f'attachment; filename="yt_{safe_id}.mp3"'})
...
def home():
    body = """
<div class="sec"><h2>MY APPS</h2>
  <a href="/chats">&#9993; Telegram</a>
  <a href="/youtube">&#9654; YouTube Audio</a>
  <a href="/search">&#128269; Search Chats</a>
</div>"""
